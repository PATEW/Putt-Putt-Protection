...
class Ball:
    def __init__(self):
        ...
        self.start_x = self.start_y = self.drag_start_x = self.drag_start_y = (
            self.velocity_x
        ) = self.velocity_y = 0
    ...
    def position_ball_in_center(self):
        ...
        center_x, center_y = (screen_width - self.ball.winfo_reqwidth()) // 2, (
            screen_height - self.ball.winfo_reqheight()
        ) // 2
    ...
    def launch_ball(self):
        ...
        new_x = min(
            max(0, self.window.winfo_x() + self.velocity_x),
            screen_width - self.ball.winfo_width(),
        )
        new_y = min(
            max(0, self.window.winfo_y() + self.velocity_y),
            screen_height - self.ball.winfo_height(),
        )
